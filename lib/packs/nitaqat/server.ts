/**
 * Nitaqat Pack - Server-Side Execution
...
import type { NitaqatInputs } from './index';
...
/**
 * Run Nitaqat analysis (server-side only)
 */
export async function runNitaqatAnalysis(
  _docText: string,
  ...
  try {
    ...
    if (!headcount || headcount <= 0) {
      return {
        ...
        errors: ['Headcount is required and must be positive'],
      };
    }

    const currentPct = currentSaudiPct ?? 0;

    // Calculate band details
    const details = calculateBandDetails(sector, headcount, currentPct);
    ...
    checklist.push({
      ...
      description: `Your company (${details.sectorName}, ${headcount} employees) has a current Saudization rate of ${currentPct.toFixed(1)}%. The minimum green band target is ${details.targetPct}%.`,
    ...
    if (details.band === 'red' || details.band === 'yellow') {
      checklist.push({
        ...
        description: `You are ${Math.abs(details.gap).toFixed(1)}% below the green band minimum. A structured hiring plan is critical.`,
        status: 'fail',
        criticality: 5,
    ...
    console.log('[Nitaqat Server] Fetching citations for checklist items...');
    for (const item of checklist) {
      const query = `${item.title} ${item.description}`.substring(0, 500);
      item.citations = await fetchCitations(item.key, query, 2);
      
      if (item.citations.length > 0) {
        console.log(`[Nitaqat Server] ✓ Found ${item.citations.length} citation(s) for ${item.key}`);
      }
    }
    ...
    return {
      ...
      checklist,
      packVersion: 'v2025.10',
      summary: `Nitaqat Analysis: ${details.band.toUpperCase()} band with ${currentPct.toFixed(1)}% Saudization. ${
        details.gap > 0 
    ...
    console.error('[Nitaqat Server] Analysis error:', error);
    return {
      status: 'failed',
      score: 0,
      checklist: [],
